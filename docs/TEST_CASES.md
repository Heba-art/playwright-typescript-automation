...
|---|---|---|---|---|---|---|---|---|
| TC-UI-001 | Registration | Register New User | High | Positive | User email is not registered | 1. Open the application<br>2. Select Signup/Login<br>3. Enter a unique name and email<br>4. Select Signup<br>5. Complete the registration form<br>6. Select Create Account<br>7. Verify `ACCOUNT CREATED!`<br>8. Select Continue<br>9. Verify `Logged in as Heba QA`<br>10. Select Delete Account<br>11. Verify `ACCOUNT DELETED!`<br>12. Select Continue | Account is created successfully, the user is logged in, and the test account is deleted during cleanup | ✅ Pass — Automated on Chromium, Firefox and WebKit (3/3 passed) |
